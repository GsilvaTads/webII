...
export function createSeededRepositories() {
    const kanban1 = [
        Column.create(COLUMN_TODO, 'A Fazer', 1),
        Column.create(COLUMN_DOING, 'Em Andamento', 2, 3),
        Column.create(COLUMN_DONE, 'Concluído', 3),
    ];
    const board1 = Board.create('board-1', 'Kanban I', kanban1);
    // Segundo quadro para testar a Atividade 11
    const kanban2 = [
        Column.create('col-mkt-1', 'Ideias', 1),
        Column.create('col-mkt-2', 'Publicado', 2),
    ];
    const board2 = Board.create('board-2', 'KanbanII', kanban2);
    // Passando os dois quadros para o repositório
    const boardRepository = new InMemoryBoardRepository([board1, board2]);
    const cardRepository = new InMemoryCardRepository();
    for (const seedCard of SEED_CARDS) {
        cardRepository.save(Card.create(seedCard.title, seedCard.columnId, seedCard.priority, seedCard.description ?? ''));
    }
