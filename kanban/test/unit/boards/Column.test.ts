...
describe('Column#restore e toSnapshot', () => {
  it('restaura uma coluna a partir do snapshot', () => {
    const snapshot = { id: 'col-9', name: 'Em Andamento', order: 3, wipLimit: 4 };
    const column = Column.restore(snapshot);

    expect(column).toBeInstanceOf(Column);
    expect(column.toSnapshot()).toEqual(snapshot);
  });
